...
function createDecryptReveal(
  ...
  function render() {
    ...
    gl!.uniform1f(u.uScrambleSpeed, Math.min(Math.max(config.scrambleSpeed, 0), 30));
  ...
  function frame(now: number) {
    ...
    const churning =
      (config.scramble > 0 && config.scrambleSpeed > 0) || (pointer.active > 1e-3 && config.edgeFlicker > 0);
...
export function DecryptReveal({ children, className, style, ...options }: DecryptRevealProps) {
