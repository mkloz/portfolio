/* eslint-disable no-empty, react-refresh/only-export-components, react/no-unknown-property */
'use client';
...
export function createBend(elements: BendElements, options: BendOptions = {}): BendInstance | null {
  ...
  function frame(now: number) {
    ...
    const phiTarget =
      reducedMotion || config.tumble <= 0 ? 0 : Math.tanh(over / 500) * 0.4 * Math.min(config.tumble, 1);
  ...
  function onClick(event: MouseEvent) {
    ...
    const point = localToClient(mapped.x, mapped.y, rect);
    if (Math.hypot(point.x - event.clientX, point.y - event.clientY) < 1.5) return;
    event.preventDefault();
    event.stopPropagation();
    const target = document.elementFromPoint(point.x, point.y);
    if (!target) return;
  ...
  return {
    setOptions(next) {
      if (!Object.entries(next).some(([key, value]) => config[key as keyof BendOptions] !== value)) return;
