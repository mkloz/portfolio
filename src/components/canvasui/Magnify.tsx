...
function createMagnify(elements: MagnifyElements, options: MagnifyOptions = {}): MagnifyInstance | null {
  ...
  function syncReadout(now: number) {
    ...
    readout.style.opacity = show ? String(Math.min(presence, 1) * Math.min(config.hud, 1)) : '0';
    if (!show) return;
    ...
    const ry = Math.min(Math.max(posY - 34, 8), Math.max(output.clientHeight - 90, 8));
  ...
  function render() {
    ...
    for (let i = 0; i < Math.min(ripples.length, MAX_RIPPLES); i++) {
      ...
      rippleData[i * 4 + 2] = (ripple.r0 + Math.max(config.rippleSpeed, 1) * ripple.age) * dpr;
  ...
  function onWheel(event: WheelEvent) {
    ...
    const unit = event.deltaMode === 1 ? WHEEL_LINE : event.deltaMode === 2 ? output.clientHeight : 1;
  ...
  return {
    setOptions(next) {
      if (!Object.entries(next).some(([key, value]) => config[key as keyof MagnifyOptions] !== value)) return;
...
export function Magnify({ children, className, style, ...options }: MagnifyProps) {
  ...
  useEffect(() => {
    ...
    instanceRef.current = createMagnify({ source, content, output }, initialOptions);
