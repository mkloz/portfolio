...
function createGrid(elements: GridElements, options: GridOptions = {}): GridInstance | null {
  ...
  return {
    setOptions(next) {
      if (!Object.entries(next).some(([key, value]) => config[key as keyof GridOptions] !== value)) return;
...
export function Grid({ children, className, style, ...options }: GridProps) {
  ...
  useEffect(() => {
    ...
    instanceRef.current = createGrid({ source, content, output }, initialOptions);
