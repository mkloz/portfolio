...
- **Transition language:** first entry resolves like Decrypt Reveal, viewport entry condenses like Particle Reveal/Particle Scroll, and route snapshots fold with Bend geometry before the next page decrypts into focus. Native document scrolling and semantic source order remain untouched; reduced-motion removes every transitional field.
- **Section continuity:** ordinary section headings remain visually fixed. Each major instrument resolves as one clipped surface when it first enters the reading window, while the hero title keeps the only typographic signal split.
